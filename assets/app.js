...
function renderBagDetail(familyId) {
  ...
  const variants = familyVariants(family);
  const publishedVariantIds = new Set(family.documented_variant_ids || []);
  const unpublishedLeads = (state.bags || []).filter(item => item.family_id === family.id && !publishedVariantIds.has(item.id));
  ...
  container.innerHTML = `<a class="back-link" href="#bags" data-route>← Back to the Catalog</a>
    ...
    <section class="family-section variants-section" aria-labelledby="variants-title"><div class="subsection-heading"><div><p class="eyebrow">Hero-backed source-documented identities</p><h2 id="variants-title">Exact Variants</h2></div><p>${plural(variants.length, "variant")}${unpublishedLeads.length ? ` · ${plural(unpublishedLeads.length, "unpublished lead")} retained in Reviews &amp; receipts` : ""}</p></div>${renderVariantCards(family)}${unpublishedLeads.length ? '<p class="fineprint variant-lead-note">Unpublished leads stay in the receipt trail until their identity and media meet the exact-variant publication bar.</p>' : ""}</section>
...
function renderResearch() {
  ...
  document.querySelector("#research-results").innerHTML = `<section class="research-section contribution-section" aria-labelledby="research-contribution-title"><p class="eyebrow">Public contribution paths</p><h2 id="research-contribution-title">Contribute to the research</h2>${renderContributionCards(["suggest-bag", "submit-source", "correction-media-removal"])}</section>
    <section class="research-section"><div class="subsection-heading"><div><p class="eyebrow">Dated campaigns</p><h2>How this corpus grew</h2></div></div><div class="auto-grid">${campaignMarkup}</div></section>
